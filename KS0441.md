...
![](media/b604495cfbf010634c8a7e91536302d2.jpg)
...
# Getting Started With Mixly Software
...
Separately connect the white LED module to the pin G, 5V and D9 of keyestudio motor drive shield V2 using three F-F jumper wires. Then stack the motor drive shield V2 onto the UNO R3 control board.

![image-20230413080613352](./media/image-20230413080613352.png)

**Note:** stack the motor drive shield on the UNO R3 board; connect white LED module to motor drive shield (pin G for GND, V for 5V, S for digital pin9 (S)).
...
Try making the LED blink without using library. Set the LED signal to D10, and turn on for 0.5 second; off for 0.2 second, alternately and circularly.
...
**Note:** stack the motor drive shield onto UNO R3 control board. connect the left obstacle detector sensor to P4（G、V、A1) connector on the motor drive shield; the right obstacle detector sensor to P5（G、V、A2) connector. If the digital ports are not enough, analog port can be used as digital port. Analog port A1 corresponds to digital port15; A2 corresponds to digital port16.
...
of the left infrared avoiding sensor (A1); using the block ![](./media/03b12fb17dfd268b28f875d7e5a4e37d.png)also makes sense.
...
**Note:** stack the motor drive shield onto UNO R3 control board; connect the pin (G、V、S) of power amplifier module to the pin G, 5V, D10 of motor drive shield V2 with 3P female-to-female jumper wire. Connect the power supply to BAT connector.  
...
In the code 1, we use the library![](media/091afe7d4355379fe58bc8ef716a4a67.png), the signal pin of passive buzzer module is connected to D10, with a frequency of 440Hz tone.
...
3.  Driving part input voltage (recommended): DC 7-9V
...
|                 | **D4** | **D5(PWM）** | **Motor B(left）** | **D2** | **D6(PWM）** | **Motor A(right）** |
|-----------------|--------|--------------|--------------------|--------|--------------|---------------------|
...
We go to write the code for motor A, B to turn front. Go to **“In/Out”**, drag out the block![](media/9eab1530ba9b3e19112700c7fdd531b8.png)and![](media/3c60b632b8e3ff6c0cc383055679f371.png); separately duplicate these two blocks, set to digital pin 4 and pin 2, change the analog pin 3 to digital pin 5 and 6. Pin 4 and pin 2 set to HIGH; pin 5 and pin 6 are PWM pin. The greater the PWM value, the faster the speed is. So here we assign both value to 100.
...
Set the pin 4 and pin 2 to LOW, assign both pin 5 and pin 6 to 100, so that the motor A, B will turn backward, thus the robot will turn back.

![](./media/a5a4424a5741c79466d8e87553b97f70.png)

Set the pin 4 to LOW and pin 2 to HIGH, assign both pin 5 and pin 6 to 100, so that the motor A turns back and motor B turns front, thus the robot will rotate to left.

![](./media/d0f0e9b3f2bb7c50c5b1b651e6895362.png)

Set the pin 4 to HIGH and pin 2 to LOW, assign both pin 5 and pin 6 to 100, so that the motor A turns front and motor B turns back, thus the robot will rotate to right.

![](./media/81fc6d993b1ad2fe5706188661cecb7f.png)

Go to **“In/Out”** again, drag out the block![](media/3c60b632b8e3ff6c0cc383055679f371.png)and duplicate once, change the pin3 to pin 5 and pin 6, assign the value 0; then add a delay block 2000ms.
...
**Note:** stack the motor drive shield onto UNO R3 control board. connect the line tracking sensor to motor drive shield’s P1 connector (G, V, D11, D7, D8);

Connect the pin (G、V、S) of white LED module to the pin G, 5V, D9(S) of motor drive shield with 3P female-to-female jumper wire. Connect the power supply to BAT connector.
...
In the code, we use the library![](media/c40a604ef2aa7ab62c3eab93b8a01729.png) to read the HIGH/LOW of the left sensor (D11); using the block![](./media/c9085683f10e49b84b5f7449b8e52679.png)also makes sense.
...
![](./media/2a324860adbb35025e9658d969743403.PNG)

**Note:** connect the infrared receiver sensor to P3（G、V、D3) connector on the motor drive shield. If the digital ports are not enough, analog port can be used as digital port. Analog port A0 corresponds to digital port14; A1 corresponds to digital port15.

The white LED module is connected to motor drive shield; pin G for GND, V for 5V, S for digital pin9 (S). Connect the power to BAT connector.
...
**![8](media/124.png)Connection diagram:**

![33](media/12d101d505db43bd833a57d2b65aaa84.JPEG)
...
Connect the left obstacle detector sensor to the P4（G、V、A1) connector on the motor drive shield; the right obstacle detector sensor to P5（G、V、A2) connector with 3P jumper wire;
...
**![4](./media/887692a3a27139fd66582d4eebf3c76a.png)**
...
**Note:** stack the motor drive shield onto UNO R3 control board. connect the line tracking sensor to motor drive shield’s P1 connector (G, V, D11, D7, D8); respectively connect the motor A and B to connector A and B on the motor drive shield. Connect the power supply to BAT connector.
...
**Note:** Stack the motor drive shield onto UNO R3 board. Due to IR receiver sensor inputting the digital signal, connect the infrared receiver sensor to P3（G、V、D3) connector on the motor drive shield. Respectively connect the
...
Drag out the block![](media/86c719c9f837abbb5ab72813e7020642.png), then click the blue gear icon, appear the edit box![](media/baf37c82c6a8781bfe430dfbbf11037d.png), drag the![](media/5679bab3e3398b3cbf7a79373b6045a9.png)block into![](media/f592e3bf0d437ef410e2932bcd2c6805.png)block, turning into![](media/18009cfb6db3423bf9241b76b6e7dd9d.png).
...
**Note:** stack the motor drive shield onto UNO R3 control board; connect the line tracking sensor to motor drive shield’s P1 connector (G, V, D11, D7, D8); connect the ultrasonic sensor to motor drive shield’s P2 connector, VCC pin to
V, Trig pin to digital 13 (S), Echo pin to digital 12 (S), G pin to GND(G);

Connect the pin (G、V、S) of power amplifier module to the pin G, 5V, D10(S) of motor drive shield with 3P female-to-female jumper wire;

Connect the left obstacle detector sensor to P4（G、V、A1) connector on the motor drive shield; the right obstacle detector sensor to P5（G、V、A2) connector;

Connect the infrared receiver sensor to P3（G、V、D3) connector on the motor drive shield;
...
So here we set 12 case statements; click the blue gear icon, appear the edit box![](media/ea015fe4513ec819c5fb4b9e6f54e32e.png), drag the![](media/5679bab3e3398b3cbf7a79373b6045a9.png) block into ![](media/f592e3bf0d437ef410e2932bcd2c6805.png) block eleven times again.
...
Duplicate the block![](media/d54e2863406f1950dc0a03e90853bac0.png)  once and drag into the second **else if** statement; set to **distance＞5 and val_L=1 and val_R=0**
